...
/// `(surface_id, codec_context_id)` — per MS-RDPEGFX, `codec_context_id` is scoped to the
/// surface that owns it, so two surfaces may reuse the same value independently.
...
impl ProgressiveDecoder {
    ...
    ///   to this surface (RDPGFX_DELETE_ENCODING_CONTEXT_PDU carries both together)
